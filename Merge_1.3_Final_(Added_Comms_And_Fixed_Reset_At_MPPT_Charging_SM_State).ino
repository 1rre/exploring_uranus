...
float open_loop=0.2; // This is the duty_cycle_set - VNotes - set initially to 0 for safety reasons
float current_mA;
unsigned int sensorValue0,sensorValue1,sensorValue2,sensorValue3;  // ADC sample values declaration

boolean RESET = 0;

float Max_Power_Duty,Adjusted_Duty; //The duty cycle at MPPT (and the adjusted duty from the DCM avoidance system)
float Max_Power,Inst_Power,Out_Power,P_Trial_1,P_Trial_2,P_Trial_3;  //The power at MPPT
boolean Power_Cond_Met=0,Move_Reset=0,From_Left=0,From_Right=0,mppt_finalised=0; //VNotes - When that becomes high, that means that a new MPP has been deduced
int Count_Up=0,Count_Down=0,DCM_A_Count=0;//At each trigger loop high (at each if statement iteration) - calculation the number of  
long REST_Count=0; 
float V_PV,V_PV_R,I_PV,V_PD_R,V_PD_L,V_PD_OUT,V_OUT,I_OUT;
float Res_Value=2.2; //This is the value of the (high power rated) resistor used in the input of the SMPS (MPPT Device) that serves as a current sensing resistor
int count=0; //VNotes - That is the number of iteration inside the mppt function (when that is ==2 (2 times SL charging SM loop) we impement the next state of mppt and sampling of smppt whereas when ==1 we implement current state mppt next state mppt )
...
int nu_operational_cells=1; //By default this is set to 3 (assuming that initialy all the cells are operational)
...
int state_num=0,next_state,total_cycles=0; //VNotes adding total_cycles measurement for a single cell
String dataString;
...
int current_cell=0; //0 for just the very beginning where the FL will occur once the interrupt occurs
float rest_current;
...
int state_num_cell_1=0,state_num_cell_2=0,state_num_cell_3=0; //The current state of the "s.m. since different implementation than the reset state machines" of each cell pre-initialized with 0 for the very beginning (1-->measuring,2-->normal,3-->discharge,4-->error
...
bool cell_1_error=0, cell_2_error=0, cell_3_error=0; //We assume that all cells start with no error
float cell_1_soc=0,cell_2_soc=0,cell_3_soc=0; //not in %
float cell_1_soh=1,cell_2_soh=1,cell_3_soh=1; //assumming that initially all cells had 100% state of health
...
int soc_int_1,soc_int_2,soc_int_3; //used to print the socs percentages as an ints and not a floats
int soh_int_1,soh_int_2,soh_int_3; //used to print the sohs percentages as an ints and not a floats

//----------------------------For-SOH-Deduction-------------------------------
int cell_that_terminated=0; //Used by terminator to be checked first logic
float soh_error_threshold=0.70; //The SOH must be at least 0.8 for the cell to be declared as a cell with low soh 

//----------------------------Communication-------------------------------
byte char_0,char_1,char_2,char_3,char_4,char_5,char_6,char_7; //bytes to be sent at control
byte dummy_bit_0,dummy_bit_1,dummy_bit_2,dummy_bit_3,dummy_bit_4,dummy_bit_5,dummy_bit_6,dummy_bit_7;
//--------------------------------------------------------------------------------------
...
//---------------------------------------------------------------
  ...
  TCA0.SINGLE.PER = 999; //VNotes Change (from 999 to 9999) Thus 100Hz from 1kHz (from T=1ms to T=0.01s)
...
void loop() {
  ...
  if (loop_trigger == 1){ // FAST LOOP (1kHz) - VNotes f=100Hz | T=0.01s
      ...
      Serial.println("------------------------------Charging/Cells State Machines Status----------------------------------------");
...
char_0=dummy_bit_0 & dummy_bit_1&dummy_bit_2 & dummy_bit_3 & dummy_bit_4 & dummy_bit_5; //Bitwise and
...
//all the states sent for all state machines will be for the very current moment that we send them
if(state_num_mppt==0){
dummy_bit_0=B00000000;
}
...
Serial1.write(char_7);
  ...
  //-------------------------------------------------------------------------------
      ...
      //That if is only useful for the very first iteration of the FL, for all the other times else will be used

      if(current_cell==0){//Think of it as the very beginning of the measurements (when we initally run the code) - Because of this the very first dt_cell_1 calculated for the very first two measurements wont be accurate! (considering that the first time of the next one wont be accurate as well) - Fixed!
      ...
      Serial.println("-------------------------Current Cell Measurement Status----------------------------------");
      ...
      if(current_cell==1){
      V_Cell_1 = analogRead(A0)*4.096/1.035; //check the battery voltage (1.03 is a correction for measurement error, you need to check this works for you) //VNotes tuned to 1.035 (With use of battery PCB nu=1)
      V_Bat=V_Cell_1;
      if ((V_Cell_1 > 3700 || V_Cell_1 < 2400) || cell_1_soh<0.7) { //Checking for Error in the cell
          cell_1_error=1; //Indicate that cell 1 has an error
      }
      else { //VNotes else for error exit condition
        cell_1_error=0; //Indicate that we remain/(or go - exit error state) to measuring state (of cell s.m.)
      }
    }

     if(current_cell==2){
      V_Cell_2 = analogRead(A1)*4.096/1.035; //check the battery voltage (1.03 is a correction for measurement error, you need to check this works for you) //VNotes tuned to 1.035 (With use of battery PCB nu=1)
    if ((V_Cell_2 > 3700 || V_Cell_2 < 2400) || cell_2_soh<0.7) { //Checking for Error in the cell
          cell_2_error=1; //Indicate that cell 1 has an error
      }
      else { //VNotes else for error exit condition
        cell_2_error=0; //Indicate that we remain/(or go - exit error state) to measuring state (of cell s.m.)
      }
    }

      if(current_cell==3){
      V_Cell_3 = analogRead(A6)*4.096/1.035; //check the battery voltage (1.03 is a correction for measurement error, you need to check this works for you) //VNotes tuned to 1.035 (With use of battery PCB nu=1)
    if ((V_Cell_3 > 3700 || V_Cell_3 < 2400) || cell_3_soh<0.7) { //Checking for Error in the cell
          cell_3_error=1; //Indicate that cell 1 has an error
      }
      else { //VNotes else for error exit condition
        cell_3_error=0; //Indicate that we remain/(or go - exit error state) to measuring state (of cell s.m.)
      }
    
    }
    
    //------------------------- COMPLETE ERROR-CHECKING & OPEATIONAL CELLS DEDUCTION & CHANGE OCCURED MONITORING--------------------------------

      if (cell_1_error==1 || cell_2_error==1 ||cell_3_error==1){ //If we have at least one error
      nu_operational_cells=2;

      if ((cell_1_error==1 && cell_2_error==1) || (cell_2_error==1 && cell_3_error==1) || (cell_1_error==1 && cell_3_error==1)){ //If we have at least a pair of error
      nu_operational_cells=1;
      }
      
      if (cell_1_error==1 && cell_2_error==1 && cell_3_error==1){ //Checking for Error states (just battery voltage for now)
    ...
    if(nu_operational_cells!=prev_nu_operational_cells){
      change_occured=1; //Indicates that there was a change in the number of operational cells
    ...
    prev_nu_operational_cells=nu_operational_cells; //Set the previous number of operational cells=current number of operational cells
    //-----------------------------------------------------------------------


      rest_current_measure(); //Measure the current obtained from the rest cells (the one measured is like it was disconnected)
      ...
      if(current_cell==1){
        ...
        total_current_measure(); //The total current will be measured here where all the relays are low
        dt_cell_1=time_final_cell_1-time_initial_cell_1; //The dt used here will serve as an ~ of the time interval passed from the past current measurement (durrent current == measurements) to the present (actually the total_current measurement but it will be precise enough) - (will be used in coulomb counting) 
        time_initial_cell_1=millis();

        Prev_I_Cell_1=I_Cell_1;//Store the previous I_Cell_1 that we had (will be used in Coulomb Counting)
        ...
        Prev_I_Cell_2=I_Cell_2; //Store the previous I_Cell_2 that we had (will be used in Coulomb Counting)
        ...
        Prev_I_Cell_3=I_Cell_3; //Store the previous I_Cell_3 that we had (will be used in Coulomb Counting)
...
count_for_mppt++; //count how many interrupts since this was last reset to zero (once we set the )
  ...
  if (int_count == 4) { // SLOW LOOP (1Hz) //VNotes changed count to 100 ensuring 1Hz | T=1s instead of 10s //Was set to 4 to allow at least! measuring each cell 1 time every time
    input_switch = !digitalRead(2); //get the OL/CL switch status (we want to make sure that when input_switch==1 we will have RESET=1 otherwise we will have RESET=0)
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 1:{ // Charge state (250mA and a green LED)
        ...
        if(soc_look_up==1)
        {//Only for the first time we get into charging state machine state 1, we will deduce the soc of each cell using voltage lookup
          ...
          soc_look_up=0; //set it to zero such that we will continue with Coulomb counting (only the mppt state of charging state machine resets it to 1)
        }
        ...
        if ((V_Cell_1>=3600 && cell_1_error!=1) || (V_Cell_2>=3600 && cell_1_error!=1) || (V_Cell_3>=3600 && cell_1_error!=1)) { // if charged, go to charge rest //make sure you do not implement any decision based on the voltage of a cell with error
          next_state = 2;
        ...
        else if(change_occured==1 | time_for_mppt==1){ //While being in charging state in the charging state machine, either of these conditions, if it's high, will make next_state (charging sm)=6 such that we will move to mppt
          next_state = 6;
      ...
      case 2:{ // Charge Rest, green LED is off and no current //VNotes Blue
        ...
        balancing_off(); //Turn off all the dischargers from all the cells (actually think of it as make sure that all or off)

        //Here the sohs will be updated and checked

        if(cell_that_terminated==1){//No need to recheck whether the cell has an error since that has already been checked from before
        ...
        else if(cell_that_terminated==2){//No need to recheck whether the cell has an error since that has already been checked from before
          cell_2_soh=cell_2_soc;
        ...
        else if(cell_that_terminated==3){//No need to recheck whether the cell has an error since that has already been checked from before
          cell_3_soh=cell_3_soc;
      ...
      case 3:{ //Discharge state //VNotes both red and green (light) on.
        ...
        cell_3_soc=soc_coulomb_counting(dt_cell_3,Prev_I_Cell_3,cell_3_soc);
         ...
         if ((V_Cell_1<=2500 && cell_1_error!=1) || (V_Cell_2<=2500 && cell_1_error!=1) || (V_Cell_3<=2500 && cell_1_error!=1)) { // if discharged, go to discharge rest //make sure you do not implement any decision based on the voltage of a cell with error
           next_state = 4;
      ...
      case 5: { // ERROR state RED led and no current
        ...
        digitalWrite(7,true);
        digitalWrite(8,false);
        if(input_switch == 0 || cond_met == 1){ //UNLESS the switch = 0, then go back to start //VNotes Error exit capability
          next_state = 0;
        ...
        case 6: { //MPPT STATE
          ...
          soc_look_up=1; //Re-initialize voltage lookup for cells socs for the very first time we get into charging state machine charging (1) state
          ...
          current_ref=0;
          next_state=6; //remain in the mppt state until the current ref is deduced (mmpt_finalised becomes high)
          //---------------------------------------
          ...
          //-------For the charging state machine----------------------
          current_current_ref=deduced_current_ref*1000; //Update the current current ref with the new deduced current ref from the mppt (*1000 to turn the A used in mppt_d to mA used by the charging sm)
          next_state=1; //regardless if we went to mppt state from reset or charging we will go to charging state (1)
    ...
    dataString = String(state_num) + "," + String(V_Cell_1) + "," + String(V_Cell_2) + "," + String(V_Cell_3) + "," + String(current_ref) + "," + String(current_measure) ; //build a datastring for the CSV file  
...
void cell_balancing(float cell_1_soc, float cell_2_soc, float cell_3_soc){ //we have to make sure that we are not turning discharge on on a cell with error

  if (((cell_1_soc - cell_2_soc > threshold_cell_difference) && cell_1_error!=1 && cell_2_error!=1) || ((cell_1_soc - cell_3_soc > threshold_cell_difference) && cell_1_error!=1 && cell_3_error!=1)) {
    ...
    if (((cell_2_soc - cell_1_soc > threshold_cell_difference) && cell_2_error!=1 && cell_1_error!=1) || ((cell_2_soc - cell_3_soc > threshold_cell_difference) && cell_2_error!=1 && cell_3_error!=1)) {
    ...
    if (((cell_3_soc - cell_2_soc > threshold_cell_difference) && cell_3_error!=1 && cell_2_error!=1) || ((cell_3_soc - cell_1_soc > threshold_cell_difference) && cell_3_error!=1 && cell_1_error!=1)) {
...
void mppt_function(){
 ...
 pwm_modulate(open_loop); // and send it out
  ...
  Serial.println("------------------------------MPPT State Machine Status----------------------------------------");
  Serial.print("The Current State of MPPT State Machine Is: ");
  Serial.println(state_num_mppt);

  if(count==2){//To make sure we have given enough time for the changes to have been implemented during the pwm_modulate
  ...
  Serial.println(open_loop);
   ...
   Serial.println("---------------------------NEW_MPPT//STATE------------------------------------");
  

  
  if(nu_operational_cells==0){//If no cells left operating then there is no point of applying MPP
  ...
  if(state_num_mppt==0){
  ...
  }//This ensures that if during one of the other states the nu_operational_cells changes the max_current_restriction will still be adjusted (midway)
    ...
    if(((Count_Up>0 || Count_Down>0) && Out_Power>(Max_Power+0.015)) && restriction_met==1){ //Ensuring that we are either in states 1 or 2
    ...
    switch (state_num_mppt){
          case 0:{
            ...
            if(RESET==0){
              ...
              Serial.println(Max_Power_Duty); Max_Power=Out_Power; 
            }
            else{
             next_state_mppt=0;     
            }
          ...
          case 2:{
            ...
            if(Count_Down==1||(Power_Cond_Met==1 && restriction_met==1)){
              ...
              open_loop=Max_Power_Duty;
              From_Right=1;
              next_state_mppt=3; //There is no reason to go back to 1 - since we just left, therefore go to the DCM avoidance state 
              Count_Down=0;
            }
          ...
          case 3:{
            ...
            if(DCM_A_Count==1){
              ...
              DCM_A_Count=0;
                ...
                if((P_Trial_1<(P_Trial_2+0.70) && P_Trial_1>(P_Trial_2-0.70) && P_Trial_1<(P_Trial_3+0.70) && P_Trial_1>(P_Trial_3-0.70)) || P_Trial_2<(P_Trial_1+0.70) && P_Trial_2>(P_Trial_1-0.70) && P_Trial_2<(P_Trial_3+0.70) && P_Trial_2>(P_Trial_3-0.70) || P_Trial_3<(P_Trial_1+0.70) && P_Trial_3>(P_Trial_1-0.70) && P_Trial_3<(P_Trial_2+0.70) && P_Trial_3>(P_Trial_2-0.70)){
                  ...
                  deduced_current_ref=I_OUT; //That current will be used as a reference current in the charging state machine.
                }
                  ...
                  if(From_Left==1){
                    ...
                    open_loop=open_loop+0.01; //Do the opposite we did before | There is no need to store this and revert to the original value since we assume this is an accurate choice and we want deliberately to result with that chose value for open_loop as initial value for future (auto - not manual) resets
          ...
          break;
            ...
            case 4:{
              ...
              next_state_mppt=0;//put that to 0 such that when we will reach the mppt state (in the charging state machine) the mppt state machine will again start with state_num_mppt=next_state_mppt=0 as it should
              mppt_finalised=1;
              break;
            ...
            case 5:{
              ...
              if(!restriction_met){
                ...
                open_loop=open_loop-0.01; //Decrease the duty cycle to decrease the output voltage and thus decrease the output current 
                }
                else if(negative_current){
                Serial.println("Negative current avoidance");
                open_loop=open_loop+0.01; //Increase the duty cycle to decrease the output voltage and thus decrease the output current 
                }
                Adjusted_Duty=open_loop;
                Serial.print("Adjusted Duty: ");
                Serial.println(Adjusted_Duty);
                ...
                Serial.println("Restriction adjustment complete - Returning to mppt state reset to proceed with mppt");
...
void sampling(){
  ...
  current_mA = ina219.getCurrent_mA(); // sample the inductor current (via the sensor chip) //This will be used for I_PV
  ...
  //V_Res_R = sensorValue2 * (4.096 / 1023.0); // Convert the V_Res_R sensor reading to volts //Changed in Version 1.1 since the SMPS is used as BOOST and therefore the current sensing resistor can be used for I_PV
  
  //I_PV=(V_PV-V_Res_R)/Res_Value; //Changed in Version 1.1 since the SMPS is used as BOOST and therefore the current sensing resistor can be used for I_PV

   Serial.print("V_OUT is: ");
  Serial.println(V_OUT);

  I_OUT = current_mA/1000.0; //VNotes adjusted from mA to A (this will be used only for debugging) //Was made negative since for Boost the current flows the other way
