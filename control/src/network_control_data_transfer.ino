...
int esp_status = 0;
IPAddress server(13, 68, 147, 164); //TODO: REPLACE WITH CONSTANT IP ADDRESS FOR OUR SERVER 13.68.147.164
...
void loop() {
  ...
  assign_values();
  concatenate();
  Serial.print(DATA);   //for testing if compute(), assign_values(), and concatenate() work, hint: they don't lol
  client.print(DATA);
  client.print(DATA1);  //for testing if can send string to server and server interprets correctly, hint: they DO
...
void assign_values() {
  ...
  //D[1] = (int)data_in;     //the data read from client.read(), THIS IS ALREADY DONE IN process_data()
