...
entity alt_cusp171_avalon_mm_bursting_master_fifo is
	generic (
...
architecture rtl of alt_cusp171_avalon_mm_bursting_master_fifo is
    COMPONENT alt_cusp171_sync_vec IS
    GENERIC (
        WIDTH : INTEGER
    );
	PORT (
...
begin
	...
	if WRITE_USED = 1 generate
		...
		wdata_fifo : alt_cusp171_general_fifo
		generic map
		(
			WIDTH              => DATA_WIDTH,
	...
	if READ_USED = 1 generate
		...
		rdata_fifo : alt_cusp171_general_fifo
		generic map
		(
			WIDTH              => DATA_WIDTH,
	...
	cmd_fifo : alt_cusp171_general_fifo
	generic map
	(
    ...
    async_clocks: IF (CLOCKS_ARE_SYNC = 0) GENERATE
        has_irq: IF (INTERRUPT_USED /= 0) GENERATE
            activeirq_reg: alt_cusp171_sync_vec
